...
impl OutPoint {
    ...
    ///
    /// ```rust
    /// use peercoin::constants::genesis_block;
    /// use peercoin::network::constants::Network;
    ///
    /// let block = genesis_block(Network::Peercoin);
...
/// Peercoin transaction input sequence number.
///
...
impl Sequence {
    ...
    /// is equivalent and improves readability for those not steeped in Peercoin folklore.
    ///
    /// ## Historical note
    ///
    /// The term 'final' is an archaic Peercoin term, it may have come about because the sequence
    /// number in the original Peercoin code was intended to be incremented in order to replace a
    /// transaction, so once the sequence number got to `u64::MAX` it could no longer be increased,
    /// hence it was 'final'.
    ///
    ///
    /// Some other references to the term:
    /// - `CTxIn::SEQUENCE_FINAL` in the Peercoin code.
    ...
    /// to 512 seconds.
    ///
    /// Encoding finer granularity of time for relative lock-times is not supported in Peercoin
    #[inline]
...
impl<E> EncodeSigningDataResult<E> {
    ...
    ///
    /// ```rust
    /// # use peercoin::consensus::deserialize;
    /// # use peercoin::sighash::{LegacySighash, SighashCache};
    /// # use peercoin::Transaction;
    /// # use bitcoin_hashes::{Hash, hex::FromHex};
    /// # let mut writer = LegacySighash::engine();
    /// # let input_index = 0;
    /// # let script_pubkey = peercoin::ScriptBuf::new();
    /// # let sighash_u32 = 0u32;
    /// # const SOME_TX: &'static str = "";
...
impl Transaction {
    ...
    pub fn txid(&self) -> Txid {
        ...
        if self.version < 3 {
            self.timestamp.consensus_encode(&mut enc).expect("engines don't error");
        }
    ...
    /// is different to what is implemented in Peercoin. The computation should be the same for
...
//            println!("output: {:?}", output);
        ...
        // lock_time
        4;
//        println!("stripped {:?}, {:?}, {:?}", self, non_input_size, input_size);
//        println!("hex {:?}", encode::serialize(&self).to_lower_hex_string());
...
impl Decodable for Transaction {
    fn consensus_decode_from_finite_reader<R: io::Read + ?Sized>(r: &mut R) -> Result<Self, encode::Error> {
        ...
        let timestamp =
        if version < 3 {
            i32::consensus_decode_from_finite_reader(r)?
        } else {
            0
        };
...
mod tests {
    ...
    const SOME_TX: &str = "";
    ...
    fn test_is_coinbase () {
        ...
        let genesis = constants::genesis_block(Network::Peercoin);
        assert! (genesis.txdata[0].is_coin_base());
        let tx_bytes = hex!("");
    ...
    fn test_nonsegwit_transaction() {
        ...
        assert_eq!(format!("{:x}", realtx.txid()),
                   "34fbecaf8e6e884a9bd46126e844caad5fdf1eab628b9deb6979ca5dce52991f".to_string());
    ...
    fn test_segwit_transaction() {
        let tx_bytes = hex!(
            "03000000000101595895ea20179de87052b4046dfe6fd515860505d6511a9004cf12a1f93cac7c01000000\
        ...
        assert_eq!(format!("{:x}", realtx.txid()),
                   "8315d8d64f03f7916c6931a2e18885ed65fb84cf438679aaea441df29e50d26f".to_string());
        assert_eq!(format!("{:x}", realtx.wtxid()),
                   "0da59232e6685206d1ef3dd7d18f5322cd38640f8b328c3eb02a1881040cf63b".to_string());
    ...
    fn test_consensus_serde() {
        ...
        let json = "\"\"";
    ...
    fn test_transaction_version() {
        ...
        let tx2_bytes = hex!("");
    ...
    fn tx_no_input_deserialization() {
        let tx_bytes = hex!(
            "030000000001000100e1f505000000001976a9140389035a9225b3839e2bbf32d826a1e222031fd888ac00000000"
        );
    ...
    fn test_ntxid() {
        ...
        assert_eq!(format!("{:x}", old_ntxid), "95e78803f25ba25c146ab01d4a7283ca031a8a94df5a386e4ebf17e8db4d525c");
    ...
    fn test_txid() {
        ...
        let tx_bytes = hex!(
            "03000000000102ff34f95a672bb6a4f6ff4a7e90fa8c7b3be7e70ffc39bc99be3bda67942e836c00000000\
        ...
        assert_eq!(format!("{:x}", tx.wtxid()), "20471c8314795e561bdb558c48d6f225aa86fbddf31626cc66ea4c7f512796f0");
        assert_eq!(format!("{:x}", tx.txid()), "6132972e57ce2b4d4b0933e565b93bdb4db22548b035bbbf9f55fcfdcbed5d4c");
        assert_eq!(tx.check_weight(), Weight::from_wu(2718));

        // non-segwit tx from my mempool
        let tx_bytes = hex!(
            "03000000010c7196428403d8b0c88fcb3ee8d64f56f55c8973c9ab7dd106bb4f3527f5888d000000006a47\
        ...
        assert_eq!(format!("{:x}", tx.wtxid()), "302d5f9942c8226c2479cc71e2c3d2147a1153784a2e702217ec299042a1b17c");
        assert_eq!(format!("{:x}", tx.txid()), "302d5f9942c8226c2479cc71e2c3d2147a1153784a2e702217ec299042a1b17c");
    }
    ...
    fn test_segwit_tx_decode() {
        let tx_bytes = hex!("");
        let tx: Transaction = deserialize(&tx_bytes).unwrap();
    ...
    fn test_transaction_verify () {
        ...
        let mut spending: Transaction = deserialize(hex!("")
            .as_slice()).unwrap();
        let spent1: Transaction = deserialize(hex!("")
            .as_slice()).unwrap();
        let spent2: Transaction = deserialize(hex!("")
            .as_slice()).unwrap();
        let spent3: Transaction = deserialize(hex!("")
            .as_slice()).unwrap();
    ...
    fn txin_txout_weight_tests() {
        ...
        let txs = [
                ...
                (true, ""),
                // one segwit input (P2WSH)
                (true, ""),
                // one segwit input (P2WPKH) and two legacy inputs (P2PKH)
                (true, ""),
                // three legacy inputs (P2PKH)
                (false, ""),
                // one segwit input (P2TR)
                (true, ""),
                // one legacy input (P2PKH)
                (false, ""),
            ];
