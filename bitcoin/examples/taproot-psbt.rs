...
// We use the alias `alias bt='peercoin-cli -regtest'` for brevity.

// Step 0 - Wipe the `regtest` data directory to start from a clean slate.

// Step 1 - Run `peercoind -regtest -daemon` to start the daemon. Bitcoin Core 23.0+ is required.
...
//                  alias bt-benefactor='peercoin-cli -regtest -rpcwallet=benefactor'
//                  alias bt-beneficiary='peercoin-cli -regtest -rpcwallet=beneficiary'
//
...
use peercoin::bip32::{ChildNumber, DerivationPath, ExtendedPrivKey, ExtendedPubKey, Fingerprint};
use peercoin::consensus::encode;
use peercoin::constants::COIN_VALUE;
use peercoin::key::{TapTweak, XOnlyPublicKey};
use peercoin::opcodes::all::{OP_CHECKSIG, OP_CLTV, OP_DROP};
use peercoin::psbt::{self, Input, Output, Psbt, PsbtSighashType};
use peercoin::secp256k1::Secp256k1;
use peercoin::sighash::{self, SighashCache, TapSighash, TapSighashType};
use peercoin::taproot::{self, LeafVersion, TapLeafHash, TaprootBuilder, TaprootSpendInfo};
use peercoin::{
...
fn generate_bip86_key_spend_tx(
    ...
    let tx1 = Transaction {
        ...
        input: vec![TxIn {
            ...
            sequence: peercoin::Sequence(0xFFFFFFFF), // Ignore nSequence.
    ...
    fn create_inheritance_funding_tx(
        ...
        let next_tx = Transaction {
            ...
            input: vec![TxIn {
                ...
                script_sig: ScriptBuf::new(),
                sequence: peercoin::Sequence(0xFFFFFFFD), // enable locktime and opt-in RBF
                witness: Witness::default(),
            }],
            output: vec![],
        };
